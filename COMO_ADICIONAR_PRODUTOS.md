...
- Todos os produtos da loja ficam cadastrados na lista `PRODUTOS` dentro do arquivo [`site/products.js`](site/products.js).
- O arquivo [`site/app.js`](site/app.js) lê essa lista e gera automaticamente os cards no catálogo ([`site/index.html`](site/index.html)), o contador de peças, os filtros de categorias, busca em tempo real, os modais de detalhes e a integração com o carrinho de compras.
...
Para facilitar, já criamos e configuramos **duas seções completas de produtos** prontas para você preencher no final da lista de produtos em [`site/products.js`](site/products.js):

### 🏷️ Seção 1: Novo Produto 1
- **Localização:** Linhas ~305 de [`site/products.js`](site/products.js)
- **ID Atual:** `STV-CAM-05`
...
- **Localização:** Linhas ~340 de [`site/products.js`](site/products.js)
- **ID Atual:** `STV-BON-04`
- **Categoria Pré-definida:** `bones`
- **Drop:** `drop-02` (*Drop II: Eclipse*)
- **Tag:** `NOVO`

> 💡 **Para personalizá-los agora:** Basta abrir o arquivo [`site/products.js`](site/products.js), localizar os blocos comentados `[SEÇÃO - NOVO PRODUTO 1]` e `[SEÇÃO - NOVO PRODUTO 2]` e alterar o nome, preço, descrição e caminhos das fotos!
...
2. Salve os arquivos na pasta da categoria correspondente:
   - Camisetas: `site/img/camisetas/`
   - Bonés e gorros: `site/img/bones/`
   - Acessórios e relíquias: `site/img/acessorios/`
...
Abra o arquivo [`site/products.js`](site/products.js) no seu editor de código.
...
1. Salve o arquivo [`site/products.js`](site/products.js) (`Ctrl + S`).
2. Abra ou recarregue a página [`site/index.html`](site/index.html) no navegador (`F5`).
...
5. **Caminho das Imagens:** Verifique se o nome do arquivo da imagem bate exatamente com o arquivo salvo na pasta `site/img/` (incluindo letras maiúsculas/minúsculas e extensão `.jpg` ou `.png`).
